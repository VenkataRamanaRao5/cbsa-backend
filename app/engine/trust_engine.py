...
Escalation Logic — Why NOT periodic GAT
-----------------------------------------
The original system ran GAT periodically (every GAT_INFERENCE_INTERVAL_SECONDS).
This is suboptimal for two reasons:
  1. WASTE: If behavior is stable (trust high, drift low), running GAT every
     N seconds wastes compute on unnecessary deep analysis.
  2. LATENCY: Genuinely anomalous behavior at second 1 of the interval still
     waits until second N for GAT analysis.

The redesigned escalation is event-driven and uncertainty-based:
