"""
Prototype Engine — Research-Grade Prototype Matching, Update, and Lifecycle

This module is the core of Layer-2's prototype-based behavioral modeling.
It implements four major improvements over the original system:

1. QUARANTINE SYSTEM (replaces immediate prototype creation):
   New behavioral patterns must pass quarantine (N_MIN observations,
   temporal spread, consistency threshold) before becoming prototypes.
   See quarantine_manager.py for full protocol documentation.

2. ADAPTIVE LEARNING RATE (replaces fixed alpha = 1/(n+1)):
   Original problem: alpha = 1/(n+1) causes the update rate to approach
   zero asymptotically. A prototype with n=1000 updates at rate 0.001 —
   essentially frozen. This means a user whose behavior legitimately evolves
   cannot update their profile, and the system will persistently misidentify
   them. The new formula:
       eta(n) = eta_base * exp(-n/tau) + eta_floor
   gives fast early learning (eta ≈ 0.31 at n=0) while maintaining a
   minimum rate eta_floor=0.01 that never goes to zero.

3. QUALITY-BASED PROTOTYPE LIFECYCLE (replaces support_count-based deletion):
   Original: enforce_prototype_limit deleted prototypes with lowest support_count.
   Problem: newly created prototypes representing current behavior would be
   deleted first — exactly the prototypes that are most relevant to the user's
   current state. The new quality score:
       Q(k) = log(1+n) * exp(-lambda * age) * max(sim, 0.1)
   balances support (n), recency (age decay), and relevance (similarity).
   Low-quality prototypes (old, rarely matched, low support) are deleted first.

4. RICH OUTPUT (replaces 5-field PrototypeMetrics):
   Full 9-field output including prototype_confidence, behavioural_consistency,
   prototype_support_strength, and anomaly_indicator. All fields in [0,1].
   Layer-4 (TrustEngine) uses this complete vector without any information loss.

Adaptive Threshold Philosophy
------------------------------
The original system used global fixed thresholds (0.8 for update, ≤0.8 for new).
The gap between them (nothing in 0.3-0.8 range triggered action) was undefined.

The redesigned thresholds:
  THRESHOLD_UPDATE  = 0.75: similarity >= 0.75 -> update existing prototype
  THRESHOLD_CREATE  = 0.50: similarity < 0.50  -> route to quarantine
  Gap [0.50, 0.75]: no action — behavior is moderately similar but not enough
                    to update. Re-evaluated at next event.

This dead-zone prevents over-updating prototypes on borderline-similar events,
which would cause prototype drift under deliberate similarity-boundary attacks.
...
def _adaptive_learning_rate(support_count: int) -> float:
    ...
    Comparison with original alpha = 1/(n+1):
      - n=100:  alpha = 0.0099 ≈ eta_floor  (similar for mature prototypes)
      - n=10:   alpha = 0.091 vs eta = 0.133  (original learns slower early on)
      - n=1000: alpha = 0.001 vs eta = 0.01   (eta_floor prevents over-freezing)
    """
...
def _update_prototype(
    ...
    standard EMA variance formulation. This differs from the original system
    which used mu_old (pre-update mean), slightly overestimating variance.
...
def _quality_score(prototype: Prototype, best_similarity: float, current_time: float) -> float:
    ...
    newly created prototypes that represent current behavior — correcting the
    original system's failure to account for relevance.
