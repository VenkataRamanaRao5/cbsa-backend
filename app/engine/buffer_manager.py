...
CRITICAL DESIGN PRINCIPLE — Statistical Leakage Prevention:
  The ORIGINAL implementation called update_session_buffer() which modified
  running_mean and running_variance WITH the current event BEFORE preprocessing
  computed drift. This means both short-drift and long-drift were computed
  against statistics that ALREADY INCLUDED the current event v_t — a classic
  case of look-ahead bias / data leakage.

  Formally, the leaked computation was:
    μ_long^{t} = Welford_update(μ_long^{t-1}, v_t)   ← includes v_t
    d_long = ||μ_short - μ_long^{t}||                 ← leakage!
  ...
  This function ATOMICALLY captures the pre-update snapshot and then performs
  the buffer update. Callers receive BOTH the updated state AND the snapshot.
