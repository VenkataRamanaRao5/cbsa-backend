...
module) must use only the snapshot for metric computations.

Leakage Analysis of the Original System
----------------------------------------
Original flow:
    session_state = update_session_buffer(event)   # v_t already in running_mean
    short_drift = l2_norm(v_t, session_state.running_mean)  # LEAKAGE: mean includes v_t

The consequence: drift was systematically underestimated because the running
mean was "pulled toward" v_t before the comparison was made. In the extreme
case (single event): running_mean == v_t, so short_drift = 0 always — completely
uninformative.
...
This is intentional and correct: we want to match the current behavioral state
(including v_t) against prototypes. The distinction from drift computation is:
...
This subtle asymmetry is deliberate and mathematically justified.
"""
...
from app.engine.drift_engine import (
    _DEFAULT_SIGMA,
