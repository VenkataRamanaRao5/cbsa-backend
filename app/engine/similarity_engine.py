...
1. ALL COMPONENTS BOUNDED IN [0, 1]:
   The original composite_similarity used stability_score as a raw input
   that could exceed 1.0 (it was the mean L2 difference between consecutive
   events — an unbounded quantity). This broke the [0,1] guarantee of the
   composite score. The redesign enforces boundedness at every component.

2. EXP-MAHALANOBIS KERNEL (replaces normalize_mahalanobis):
